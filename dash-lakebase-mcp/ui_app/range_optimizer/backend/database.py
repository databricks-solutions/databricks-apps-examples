...
def initialize_connection_pool() -> bool:
    ...
    with _pool_lock:
        ...
        try:
            ...
            logger.info(f"  Resolved database: {database}")
            logger.info(f"  Resolved user: {user}")
            logger.info(f"  Resolved instance_name: {instance_name}")
            
            if not host:
                logger.error("Database host not configured - check PGHOST or LAKEBASE_INSTANCE_NAME")
                raise ValueError("Database host not configured")
            if not instance_name:
                logger.error("Instance name not configured - check LAKEBASE_INSTANCE_NAME")
