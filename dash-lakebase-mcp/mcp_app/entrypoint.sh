...
if [ "${DEV_MODE:-false}" = "true" ]; then
    ...
    exec uvicorn range_optimizer.backend.mcp_standalone:app --host 0.0.0.0 --port $APP_PORT --reload --reload-dir range_optimizer
else
    # Run the application (production mode)
    exec uvicorn range_optimizer.backend.mcp_standalone:app --host 0.0.0.0 --port $APP_PORT
fi
